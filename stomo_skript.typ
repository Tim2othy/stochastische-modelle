...
$
  f (x_1, ..., x_n) = f_(X_1) (x_1)... f_((X_n)) (x_n) "für alle" x_1, ..., x_n in RR,
$
...
$
  EE [g (X)] & = sum_X g (x) P(X = x) \
             & = sum_X g (x) sum_Y f_(X | Y) (x | y) P(Y = y) \
...
#example([Beispiel 3], subtitle: [Irrfahrt auf $ZZ$])[
  ...
  $i_n = i_(n - 1) + Y_n$ auf den ganzen Zahlen $ZZ$. Damit gilt offensichtlich für jedes $n >= 1$, dass
  $X_n = i_(n - 1) + Y_n <=> Y_n = i_n - i_(n - 1)$. \
  Für alle $n >= 0$ und alle $i_0, ..., i_(n + 1) in ZZ$ mit
...
#example([Beispiel 4], subtitle: [Sukzessive Maxima])[
  ...
  Für alle $n >= 0$ und alle $i_0, ..., i_(n + 1) in NN_0$ mit
  $P(X_0 = i_0, ..., X_n = i_n) > 0$ ist
  $
      & P(X_(n + 1) = i_(n + 1) | X_0 = i_0, ..., X_n = i_n) \
...
#example([Beispiel 5], subtitle: [Rekursive Darstellung einer Markovkette mittels zufälliger Funktionen])[
  ...
  $
  Dabei sei
  $f : S times RR -> S$, $Y_1, ..., Y_n$ seien identisch verteilte Zufallsvariablen und $Y_0, Y_1, ...$ seien
  unabhängig. \
  $=> { X_n : n in NN_0 }$ ist eine Markovkette mit Übergangswahrscheinlichkeiten $p_(i j) = P(f (i, Y_1) = j)$, denn
  für
...
#example[Beispiel 7][
  ...
  $
    ...
    & = p_0 p_02 p_22 p_24 \
    & = 1 dot.op 1 / 5 dot.op 3 / 5 dot.op 1 / 5 = 3 / 125
  $
  Was wäre die Wahrscheinlichkeit, dass $X_1 >= 2$ und $X_2 <= X_1$? \
...
$
  (p_i^((n)))_(i in S)
$
der Übergangsmatrix mit
$p_i^((n)) = P(X_n = i)$ und $n in NN_0$ entspricht, die Wahrscheinlichkeit in weiteren $m in NN_0$ Schritten ein
beliebiges
...
#example([Beispiel 9], subtitle: [Ruinproblem])[
  ...
  Betrachte hier für die Markovkette ${ X_n : n in NN_0 }$ mit Zustandsraum $S = { 0, 1, 2, ..., M }$ und
  ...
  $
  und
  $h_M = 0$ folgt für das $a$ aus der Gleichung
  $
        0 & = 1 + a dot M \
    <=> a & = - 1 / M
  $
...
#example(
  ...
  $
    p_(i j) = cases(
      1 & "falls" i = 0 "und" j = 0,
      p & "falls" i in NN_0 "und" j = i + 1,
      1 - p & "falls" i in NN_0 "und" j = i - 1,
      0 & "sonst",
    )
  $
  ...
  Dann gilt
  $
    h_i - h_(i + 1) =^((star)) Theta^i (1 - h_1) >= 1 - h_1
  $
  also
  $
...
#proof(title: [Beweis von @thm:solution_markov])[
  ...
  + Sei $(x_i)$ eine Lösung mit $X_i >= 0$, $i in S$.
    ...
    und die Teilmatrix $Q = (p_(i j))_(i, j in A^c)$ gilt durch iteratives Einsetzen
  $
    x & = r + Q x \
      & = r + Q (r + Q x) \
      & = r + Q r + Q^2 x \
...
#proof(title: [Beweis von @thm:convergence_markov])[
  ...
  $
  Mit
  $e = vec(1, dots.v, 1) in RR^S$ ist $M = e pi$ und
  $
  ...
  $
    ...
    => pi^n - M & = (1 - epsilon)^n (Q^n - M) quad forall quad n in NN_0
  $
...
#proof(title: [Beweis von @thm:convergence_markov2])[
  ...
  $
    (Pi^k)^m = Pi^(k m) = (P_(i j)^(k m))
  $
  Damit liefert @thm:convergence_markov:
  $
...
#definition[
  Die Periode $d_i$ eines Zustands $i$ einer Markovkette ist die größte ganze Zahl, die alle $n in NN$ mit $P_(i i)^n > 0$ teilt:
  $
    d_i = g g T ({ n in NN . : P_(i i)^n > 0 }) \
    \
...
#proof(title: [Beweis von @thm:periodic_markov])[
  ...
  $
  gelte also $g g t (N_i) = 1$, insbesondere $N_i != nothing$. \
...
$
  tilde(pi)_i p_(i k)^n = tilde(pi)_k / pi_k pi_i p_(i k)^n
$
Wähle $n$
mit $P_(i k)^n > 0$
$
...
#proof(title: [Beweis @thm:stationary_markov])[
  ...
  Sei $i_0 in S$
  aperiodisch. \
  $=>$ Es gibt ein $n_0 in NN$, sodass
  $
    P_(i_0 i_0)^n > 0 quad forall n >= n_0
  $
  Da $(P_(i j))$ irreduzibel ist, gibt es für alle $i, j in S$ ein $n_(i j) in NN_0$ mit
  $P_(i j)^(n_(i j)) > 0$. \
...
#example[Beispiel 15][
  ...
  Dann ist
  $
    f_11 & = sum_(n = 1)^oo P(tau_1 = n | X_0 = 1) \
...
#theorem[Satz 18][
  ...
  $
  die Anzahl der Zeitpunkte $>= 1$ zu denen Zustand $j$ besucht
  wird. \
...
#proof(title: [Beweis @thm:recurrence_transience])[
  ...
  $sup nothing = oo$. Ist $sigma_j$ endlich, dann ist $sigma_j$ der Zeitpunkt des letzten Aufenthalts in $j$. Für jedes $n in NN$ gilt
  $
  ...
  - Sei $j$ rekurrent, also $f_(j j) = 1$. \
    $=> P(sigma_j = n | X_0 = i) = 0$ für alle $n in NN$. \
...
#example[Beispiel 16][
  ...
  $
  //TODO is the denominator of (2 n) right?
  und
  $vec(2, 1) = 2$ gilt
  $
    vec(2 n, n) <= 4^n
...
#example[Beispiel 17][
  ...
  $
    N (270) = m a x { n in NN_0 : T_n <= 270 } = 2
  $
  Angenommen, wir beobachten an Tag 1000, dass die 7. Glühbirne
...
#example[Beispiel 18][
  ...
  $
    mu approx t / "Anzahl Meldungen bis Zeit t"
  $
  berechnet werden. \
  ...
  und
  $
    lim_(n -> oo) 1 / n sum_(i = 1)^n X_i = EE [X_i]
  $
  folgt mit
  $lim_(t -> oo) N (t) = oo$
  $
...
#proof(title: [Beweis @thm:renewal_distribution])[
  ...
  $x, x_1, x_2, ... in RR$ mit $lim_(n -> oo) X_n = x$ gilt
  $
    lim_(n -> oo) P(X_n > x_n) = G (x)
  $
  Sei nun $y in RR$. Für alle $t > 0$ gilt
  $
...
#exer[Aufgabe 42][
  ...
  *Hinweis zu (a).*
  Benutzen Sie eine Zufallsvariable $W$, so dass $W$ dieselbe Verteilung hat wie $U$
  und so dass $U, V, W$ unabhängig sind.
...
#exer[Aufgabe 43][
  ...
  Interpretiert man die $Y_n$ als Lebensdauern von nacheinander verwendeten
  Bauteilen, dann ist $X$ die Lebensdauer des zur Zeit $t^*$ verwendeten Bauteils.
...
#definition[
  ...
  Zeitpunkte
  $0 = t_0 < t_1 <... < t_k$ die Zufallsvariablen
  $
    & N (t_0), \
    & N (t_1) - N (t_0), \
    & dots.v \
